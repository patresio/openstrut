...
describe('Inventory', () => {
  it('inventory contains exactly 206 artifacts', () => {
    assert.equal(INVENTORY.length, 206, `Expected 206 artifacts, got ${INVENTORY.length}`);
  });
  ...
  it('3 + 40 + 10 + 11 + 10 + 127 + 0 + 4 + 1 = 206', () => {
    ...
    assert.equal(sum, 206, `Expected sum 206, got ${sum}`);
  });
});
