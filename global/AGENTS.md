...
## 1. Precedence and Repository Sources

Apply instructions in this order:

1. Global safety, integrity, privacy, evidence, and non-destructive invariants.
2. The current user request and latest explicitly approved scope.
...
6. Tool, model, framework, and style heuristics.
   ...
   Before proposing or executing changes, discover the applicable:
...
- Implementation, bugfix, and behavioral refactoring use the full workflow.
...
- Skills are procedures loaded on demand.
- Subagents are specialists used only when separation improves accuracy, speed, or context control.
- `build` owns synthesis, integration, validation, plan compliance, and final reporting.
- `plan` remains a manual read-only alternative.
  Before delegating, define objective, bounded scope, relevant plan steps, mutation policy, file ownership, expected output, evidence, and stop conditions.
...
- factual final reporting.
  Issue, branch, Archive, Commit, Push, and PR are conditional on the approved request and repository workflow.
  A narrower workflow may omit only inapplicable delivery artifacts; it may not remove safety, approval, testing, validation, review, or reporting gates.
...
Approval must explicitly authorize execution of the plan or be an unambiguous equivalent.
Silence, continued discussion, approval of one detail, or unrelated prior approval is insufficient.
...
Use the project convention. When none exists, use:
`.agents/task-plans/<task-id>.md`
...
Do not change `.gitignore` or decide whether the plan is committed without project policy or approval.
...
7. refactor only while relevant tests remain GREEN.
   Invalid RED causes include syntax errors, missing dependencies, broken setup, invalid fixtures, unrelated failures, or assertions that do not specify the approved behavior.
   For bugfixes, reproduce the defect with a failing regression test first.
   For untested legacy code, characterize relevant current behavior before changing it.
   For behavior-preserving refactoring, begin from a verified GREEN baseline; do not invent an artificial failure.
   Never manufacture GREEN by weakening, deleting, skipping, or quarantining legitimate tests; blindly updating snapshots; changing expectations to match a defect; mocking the behavior under test; reducing meaningful coverage; or bypassing the authoritative command.
   Exceptions are limited to documentation-only changes, non-behavioral comments or formatting, generated artifacts, non-executable declarative configuration, disposable unmerged spikes, and exploratory visual prototypes where automation is not yet practical.
   Declare the exception and alternative validation in the Task Plan before implementation.
...
Use acceptance criteria, an approved plan, issue, executable example, test, contract, ADR, or project-specific specification.
Use the repository's established mechanism.
OpenSpec is conditional: use it only when the project actively uses it.
Keep specifications, implementation, tests, and documentation consistent.
Use BDD-style examples for business outcomes, user journeys, state transitions, complex rules, or stakeholder-visible behavior.
Do not require a BDD framework when expressive tests or acceptance criteria are sufficient.
...
- record meaningful evidence in the Task Plan.
  ...
  When containers, CI scripts, or task runners are authoritative, validate there.
  Do not silently substitute another environment, database, service, mock, or weaker command.
  Alternate validation is not equivalent unless documented or approved.
...
Remove true knowledge duplication.
Preserve superficially similar code when it changes for different domain reasons.
...
- stop when safe isolation is not possible.
  Never automatically stash, reset, clean, checkout, restore, discard, overwrite, or rewrite history.
  Never claim ownership of pre-existing changes without evidence.
  Prefer short-lived branches and frequent integration, but do not force direct trunk development when PRs are required.
  Use worktrees only for independent parallel writes, explicit isolation, or concurrent approved tasks.
  Before removing one, verify registration, branch, cleanliness, and merged status.
...
- archive through OpenSpec when the project uses it;
...
Never hardcode or expose credentials, tokens, passwords, private keys, or sensitive personal data.
Use approved external secret sources: environment variables, protected files, OS key stores, or secret managers.
...
Permission prompts are approval gates, not operating-system sandboxing.
...
- unresolved risks, blockers, and follow-up work.
  ...
  Do not restart completed Git or validation loops after reporting completion.
